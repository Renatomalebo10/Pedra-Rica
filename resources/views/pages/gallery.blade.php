...
@section('title', 'Galeria - Pedra Rica Oficial')
...
@section('content')
    ...
    <section class="relative bg-gradient-to-br from-[#1e3a5f] via-[#1e40af] to-[#2563eb] text-white overflow-hidden">
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-28">
            <div class="max-w-3xl">
                <span class="inline-block text-sm font-semibold text-blue-200 uppercase tracking-widest mb-3">Momentos</span>
                <h1 class="text-4xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight leading-tight">Galeria</h1>
                <p class="mt-6 text-lg sm:text-xl text-blue-100/90 max-w-xl leading-relaxed">Os melhores momentos da Pedra Rica.</p>
            </div>
        </div>
        ...
        <section class="py-6 bg-white border-b border-gray-100">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('pages.gallery') }}" class="inline-flex items-center px-4 py-2 rounded-full text-sm font-semibold bg-[#1e40af] text-white shadow-sm">
                        Todos
                    </a>
                    @foreach($galleryCategories as $category)
                        <a href="{{ route('pages.gallery-category', $category) }}" class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium bg-gray-100 text-gray-700 hover:bg-blue-50 hover:text-[#1e40af] transition-colors">
                            {{ $category->name }}
                            <span class="ml-1.5 text-xs bg-white/50 px-1.5 py-0.5 rounded-full">{{ $category->images()->count() }}</span>
                        </a>
                    @endforeach
                </div>
            </div>
                        ...
                        <div class="break-inside-avoid group relative overflow-hidden rounded-2xl bg-gray-200">
                            ...
                            <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                                <div>
                                    ...
                                    @if($image->category)
                                        <span class="text-xs text-blue-200 mt-0.5 block">{{ $image->category->name }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
